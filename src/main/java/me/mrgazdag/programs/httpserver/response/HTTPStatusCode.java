...
/**
 * An enum containing all common status codes.<br>
 * 
 * <h2>1XX: informational response</h2><br>
 * An informational response indicates that the request
 * was received and understood. It is issued on a provisional
 * basis while request processing continues.
 * It alerts the client to wait for a final response.
 * The message consists only of the status line and optional header
 * fields, and is terminated by an empty line. As the HTTP/1.0 standard
 * did not define any 1xx status codes, servers must not send
 * a 1XX response to an HTTP/1.0 compliant client except under
 * experimental conditions.<br>
 * 
 * <h2>2XX: success</h2><br>
 * This class of status codes indicates the action requested
 * by the client was received, understood, and accepted.<br>
 * 
 * <h2>3XX: redirection</h2><br>
 * This class of status code indicates the client must take
 * additional action to complete the request. Many of these
 * status codes are used in URL redirection.<br><br>
 * A user agent may carry out the additional action with no
 * user interaction only if the method used in the second
 * request is GET or HEAD. A user agent may automatically
 * redirect a request. A user agent should detect and intervene
 * to prevent cyclical redirects.<br>
 *
 * <h2>4XX: client errors</h2><br>
 ...
 * condition. These status codes are applicable to any request
 * method. User agents should display any included entity to
 * the user.<br>
 * 
 * <h2>5XX: server errors</h2><br>
 * The server failed to fulfill a request.<br>
 * Response status codes beginning with the digit "5" indicate
 * cases in which the server is aware that it has encountered
 * an error or is otherwise incapable of performing the request.
 * Except when responding to a HEAD request, the server should
 * include an entity containing an explanation of the error
 * situation, and indicate whether it is a temporary or
 * permanent condition. Likewise, user agents should display
 * any included entity to the user. These response codes are
 * applicable to any request method.<br>
 * 
 * Source: <a href="https://en.wikipedia.org/wiki/List_of_HTTP_status_codes">Wikipedia: List of HTTP status codes</a>
 */
...
public enum HTTPStatusCode {
	 ...
	 */
	@Deprecated
	HTTP_306_SWITCH_PROXY(306, "Switch Proxy"),
	/**
	...
	/**
	 *  the user does not have valid authentication credentials for the target resource. Note: Some sites incorrectly issue HTTP 401 when an IP address is banned from the website (usually the website domain) and that specific address is refused permission to access a website.Similar to 403 Forbidden, but specifically for use when authentication is required and has failed or has not yet been provided. The response must include a WWW-Authenticate header field containing a challenge applicable to the requested resource. See Basic access authentication and Digest access authentication. 401 semantically means "unauthorised", 
	 */
	 ...
	 */
	HTTP_402_PAYMENT_REQUIRED(403, "Payment Required"),
	/**
